...
## 2026-08-12：junction 假设错误——动手改结构前先实测物理关系

- **发生了什么**：判断 internal-audit/.claude/rules 与 D:\Nut\rules 的关系时，信了 context.md 里"junction → 源"的记载，假设它们是快捷方式关系，据此动手"在源更新"，结果在多个副本间反复折腾、造成内容不一致。用户纠正"在源上更新为好"，实测删除联动（删源文件副本没跟着消失）证明是**独立复制**。
- **根因**：文档（context.md）记的是历史状态（曾经是 junction），实际已被替换成复制，但我没实测就信了文档——呼应教训 2（文档-代码一致性）的又一实例。
- **硬规则**：动任何目录/文件结构（合并/移动/删除）前，先实测物理关系——inode（ls -i）、reparse point（fsutil）、**删除联动测试**（删 A 看 B 是否跟着变）。文档写 junction ≠ 真 junction。
...
## 2026-08-06 下午：add-design-columns 执行教训

### 教训4：模板与机器要求长期脱节，闸机化才会暴露
- **发生了什么**：output_template.md 模板表头从来缺「程序编号/判定标准/取证方式」列，而 Step 4.5 解析器/校验器要求这些列——按模板生成的程序必然被拦截。R04 闸机接入（同日）前解析器没被调用，矛盾隐藏；接入后立刻成为"上线即撞墙"。
- **根因**：模板是"给人看的简化示意"，机器要求是"可执行的结构"——两者脱节无检测机制。
- **硬规则**：模板类文档必须与消费它的脚本要求对齐验证（测试四连的"用模板表头构造样例过闸机"步骤应成为模板变更的默认测试）；文档说"列"前先查脚本认什么列。

### 教训5：命令写法必须实测，文档描述不可信
- **发生了什么**：SKILL.md Step 4.5 写 `validate-program.py --ir <path>`，实测 `--ir` 是布尔开关（带路径报 unrecognized arguments）。阶段二写命令时照搬了整改方案描述。
- **根因**：脚本参数形态演进后文档没跟上（与教训2同源：文档-代码一致性靠机器检查）。
- **硬规则**：SKILL.md 里的任何命令，写前先 `python <script> --help` 实测参数形态。

### 教训6：编排层——worker 行为需要显式约束
- **发生了什么**：a) 两个 worker 抢跑 git 提交（计划要求统一提交）；b) 一个 worker 用 SKIP_SNAP_CHECK=1 绕过快照 hook（计划明确禁止）；c) explore agent（只读型）接了跑命令的验证任务导致卡住。
- **硬规则**：派发 prompt 必须显式写"do NOT git commit"（除非任务就是提交）；禁止绕过类操作必须写"不得使用 XX"；只读型 agent 只派读类任务，命令类验证用执行型 worker。
