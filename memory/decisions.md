...
## ADR-026
日期：2026-08-11
背景：系统规则和脚本持续膨胀——目前已有 25 个脚本 + 14 条宪法硬约束，且每次整改都在堆叠新的防御机制。防御性堆叠本身成了新的复杂度来源：规则越来越多、脚本越来越多，反而需要更多规则去守护规则。
备选方案：
- A) 不设门槛，按需求逐个新增规则/脚本
- B) 审批制，每次新增都由用户逐一审批
- C) 纳米测试原则：新增前先自问三个问题，用"更简单的机制能否解决"过滤
最终选择：C
原因：
- 600 行教学版理念：能用更简单机制解决的不需要脚本——系统复杂度的健康标准不是"能跑"，而是"最少必要"
- 25 个脚本 + 14 条宪法已经出现防御性堆叠，新增任何东西都在加固这条链，链越壮复杂度越高
- 审批制成本过高：每次新增都打断用户，无法区分"必要的加固"和"顺手堆的防御"
- 自问三问是零成本的第一道闸：先过滤掉工程噪音，真正必要的新增才值得讨论和审批
影响：新增规则/脚本前必须过"三个自问"；OPS.md 新增对应检查清单章节

## ADR-027
日期：2026-08-27
背景：开发仓 .claude/skills/ 长期兼任“部署取货点”与“技能仓库”双重身份。该混叠已引发两类事故：(1) ~/.claude 快照幽灵目录反复复活；(2) 2026-08-27 09:26 外部会话把该目录下 10 个审计技能的 junction 清成空壳，部署链断裂（真身无损）。根因：该位置长得像“可清理的重复副本”，任何工具或 AI 会话都可能再次动手。
备选方案：
- A) 恢复 junction 原状，靠规范防止再被误清
- B) 根目录设为唯一真身来源，部署脚本直接扫根目录；.claude/skills 降级为可一键重建的门牌，无任何脚本再把它当仓库读
最终选择 B
原因：
- A 不解决矛盾本体：只要门牌长得像副本就会被再次清理，两次同类事故证明“以后小心”不可依赖
- SKILL.md 标记识别法让根目录天然成为技能清单，新增技能零配置、无双源漂移可能
- 取数单源头后，历史上“stable 更新丢 10 技能”类双源不一致 bug 连发生土壤都不存在
影响：
- setup-project.ps1 / update-project.ps1 改为按“根目录含 SKILL.md 即技能”扫描，[1m开发仓 .claude/skills 仅存 geb-* 两个原生真身（用户指定），审计项目默认不再携带文档治理技能[0m
- quality_gate.py / storage.py 共三处 ~/.claude 死路径改为随脚本位置解析到 REPO_ROOT；经查幽灵目录从未生成过数据，无需迁移
- 顺带修复 storage.py / quality_gate.py 两处历史 Optional 类型标注问题（纯标注，无行为变化）
- 已知遗留：geb-bootstrap/geb-workflow 在 .agents/skills 与 .claude/skills 各存一份且内容有漂移风险（见全面审计报告_2026-07-14 L185），本次不动，留待单独任务
- 真机验收：junction 与 stable 部署各 21 OK + 项目自检 10/10；stable 升级合并路径实测 10 技能备份+升级、逐技能文件数与源完全守恒、geb 未泄漏
