...
**目的**：解决控制点规则分散在多个章节时 LLM 逐段分析无法拼接的问题。先建"业务对象"索引，再对每个对象做完整上下文分析。
...
**输出**：每个业务对象产出一个控制点分析块，LLM 拿到的是完整上下文，不是碎片。
...
### verification_status 状态机铁律（MANDATORY_RULE）

<!-- MANDATORY_RULE: verification_status 状态跃迁必须经用户显式确认 -->

**LLM 绝对禁止在没有用户明确确认的情况下，将 `verification_status` 从「待确认」改为「已确认」。**

- 「待确认」→「已确认」是一次不可逆的判定升级：一旦标为「已确认」，该缺口就可直接作为设计缺陷进入 Phase 4 验证升级路径。
- 状态跃迁的唯一合法触发是用户显式回复（如"确认缺失""已核对全部制度，确实没有"）。LLM 自行推断"应该分析完了"不构成确认。
- 允许的自动跃迁只有：新增文件分析后，`引用待追踪` → `待确认` 或 `跨文件覆盖`（这类是向下、非升级的重新归类，不触碰"已确认"）。
- 若 LLM 认为某项应升级为「已确认」，只能向用户提议并等待回复，禁止先斩后奏。
