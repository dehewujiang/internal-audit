---
name: geb-bootstrap
description: GEB 文档体系冷启动播种。当项目缺少 CLAUDE.md 或需要初始化三层文档结构时调用。会扫描目录、推断职责、播下 L1/L2/L3 并建立 symlink。
---
...
- 检查 /CLAUDE.md 是否存在；存在则读取理解，不存在则准备播种。
...
- 创建 symlink：AGENTS.md → CLAUDE.md（保证不同 Agent 工具发现同一份文档）。
